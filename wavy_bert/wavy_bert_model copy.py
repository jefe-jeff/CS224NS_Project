...
class LightningWavyBert(pl.LightningModule):
    ...
    def get_audio_tokens(self, x):
        waveforms = [x_[0] for x_ in x]
        utterances = [x_[1] for x_ in x]
        time_alignments = [x_[2] for x_ in x]
    ...
    def forward(self, x):
        embedding, hidden_states = self.encoder(x)
